...
def piecewise_cubic_spline(points):
    ...
    ## f(X1) = y1, f(X2)=y2
    for i in range(n):
        matrix[2*i][4*i:4*i+4] = [points[i][0]**3,points[i][0]**2,points[i][0],1]
        matb[2*i][0]=points[i][1]

        matrix[2*i+1][4*i:4*i+4] = [points[i+1][0]**3,points[i+1][0]**2,points[i+1][0],1]
        matb[2*i+1][0] = points[i+1][1]

    ## f1'(x2)-f2'(x2)=0
    for i in range(n-1):
        matrix[2*n+i][4*i:4*i+7] = [3*points[i+1][0]**2,2*points[i+1][0],1,0,-3*points[i+1][0]**2,-2*points[i+1][0],-1]

    ## f1''(x2)-f2''(x2)=0
    for i in range(n-1):
        matrix[2*n+(n-1)+i][4*i:4*i+6] = [6*points[i+1][0],2,0,0,-6*points[i+1][0],-2]

    ## f1''(x1)=0
    matrix[-2][0:2]=[6*points[0][0],2]
    ## f(n-1))''(x(n-1))=0
    matrix[-1][-4:-2]=[6*points[-1][0],2]
