...
def calc_geo(op, x1, n, p, x2=None):

    if op == "<":
        return sum(ideal_geo(n, i, p) for i in range(x1))
    if op == ">":
        return 1-sum(ideal_geo(n, i, p) for i in range(x1 + 1))
    if op == "≤":
        return sum(ideal_geo(n, i, p) for i in range(x1+1))
    if op == "≥":
        return 1-sum(ideal_geo(n, i, p) for i in range(x1))

    if op == "=":
        return ideal_geo(n,x1,p)
    if op == "< <":
        return sum(ideal_geo(n, i, p) for i in range(x1+1,x2))
    if op == "≤ ≤":
        return sum(ideal_geo(n, i, p) for i in range(x1,x2+1))

def backwards_calc_geo (op,n,p,pp):
    if op == "<":
        k=0
        while calc_geo(op,k,n,p)<=pp:
            k+=1
        return k-1
    if op == "≤":
        k=0
